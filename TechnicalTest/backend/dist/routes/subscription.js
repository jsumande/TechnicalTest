...
router.post('/checkout', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const customerId = await (0, stripe_1.getOrCreateCustomer)(user.email, user.stripeCustomerId);
        ...
        const checkoutUrl = await (0, stripe_1.createCheckoutSession)(customerId, process.env.STRIPE_PRICE_ID, `${frontendUrl}/subscription?success=true`, // After successful payment
        `${frontendUrl}/subscription?canceled=true`);
...
router.post('/portal', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
        const portalUrl = await (0, stripe_1.createPortalSession)(user.stripeCustomerId, `${frontendUrl}/subscription`);
